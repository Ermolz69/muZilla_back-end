...
namespace muZilla.Application.Services
{
    public class FriendsCoupleService
    {
        ...
        {
            var requests = tableClient.QueryAsync<Entities.Models.Request>(r =>
        ...
        {
            await _repository.AddAsync<FriendsCouple>(new FriendsCouple { UserId = id, FriendId = friendId });
        ...
        {
            var exists = await _repository.GetAllAsync<FriendsCouple>().Result
                .AnyAsync(fc =>
        ...
        {
            var friendsIds = await _repository.GetAllAsync<FriendsCouple>().Result
